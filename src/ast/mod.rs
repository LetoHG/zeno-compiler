...
impl Ast {
    ...
    fn struct_type(
        ...
        self.add_stmnt(ASTStatementKind::StructDecl(ASTStructStatement {
            identifier,
            members,
        }))
    }
    ...
    fn member_access(&mut self, token: Token, expr_id: ExprId) -> Option<&ASTExpression> {
        self.add_expr(ASTExpressionKind::MemberAccess(ASTMemberAccessExpression {
            identifier: token,
            member: expr_id,
        }))
    }
    ...
    fn struct_initializer(
        ...
        self.add_expr(ASTExpressionKind::StructCtor(
            ASTStructInitializerExpression {
                identifier,
...
pub trait ASTVisitor<T> {
    fn do_visit_statement(&mut self, ast: &mut Ast, statement_id: StmntId) -> T {
        ...
        match &statement.kind {
            ...
            ASTStatementKind::StructDecl(statement) => self.visit_struct_statement(ast, statement),
    ...
    fn do_visit_expression(&mut self, ast: &mut Ast, expr_id: ExprId) -> T {
        ...
        match &expr.kind {
            ...
            ASTExpressionKind::MemberAccess(expr) => self.visit_member_access_expression(ast, expr),
            ASTExpressionKind::StringLiteral(_) => todo!(),
            ASTExpressionKind::StructCtor(statement) => {
                self.visit_struct_initializer_expression(ast, statement)
            }
    ...
    fn visit_struct_statement(&mut self, ast: &mut Ast, struct_def: &ASTStructStatement) -> T;
    ...
    fn visit_variable_expression(&mut self, ast: &mut Ast, expr: &ASTVariableExpression) -> T;
    fn visit_member_access_expression(
        &mut self,
        ast: &mut Ast,
        expr: &ASTMemberAccessExpression,
    ) -> T;

    fn visit_unary_expression(&mut self, ast: &mut Ast, expr: &ASTUnaryExpression) -> T;
    fn visit_binary_expression(&mut self, ast: &mut Ast, expr: &ASTBinaryExpression) -> T;
...
//         }
//
//         fn visit_member_access_expression(&mut self, ast: &mut Ast,  _expr: &super::ASTMemberAccessExpression) {}
//
